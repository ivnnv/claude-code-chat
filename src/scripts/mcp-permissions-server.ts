...
export class PermissionsMcpServer {
  private server: McpServer;
  private permissionsPath: string;

  constructor(permissionsPath: string) {
  ...
  private getWorkspacePermissionsPath(): string | null {
    if (!this.permissionsPath) {return null;}
    return path.join(this.permissionsPath, 'permissions.json');
  }

  private loadWorkspacePermissions(): WorkspacePermissions {
    const permissionsPath = this.getWorkspacePermissionsPath();
  ...
  private isAlwaysAllowed(toolName: string, input: any): boolean {
    const permissions = this.loadWorkspacePermissions();
  ...
  private async requestPermission(tool_name: string, input: any): Promise<{approved: boolean, reason?: string}> {
    ...
    if (this.isAlwaysAllowed(tool_name, input)) {
      console.error(`Tool ${tool_name} is always allowed for this workspace`);
      return { approved: true };
    }

    const requestId = this.generateRequestId();
    const requestFile = path.join(this.permissionsPath, `${requestId}.request`);
    const responseFile = path.join(this.permissionsPath, `${requestId}.response`);
    ...
    try {
      ...
      return new Promise<{approved: boolean, reason?: string}>((resolve) => {
        ...
        const watcher = fs.watch(this.permissionsPath, (eventType, filename) => {
  ...
  private setupTools(): void {
    this.server.tool(
      ...
      async ({ tool_name, input }) => {
        ...
        const permissionResult = await this.requestPermission(tool_name, input);
  ...
  public async start(): Promise<void> {
    ...
    console.error(`Using permissions directory: ${this.permissionsPath}`);
  }
}
